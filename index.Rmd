...
author: "Yazar: [Christopher Groskopf](https://github.com/onyxfish) Metnin orijinali: [The Quartz guide to bad data](https://github.com/Quartz/bad-data-guide) Çeviren: [Serdar Balcı](https://github.com/sbalci)"
subtitle: Günlük pratikte verilerde sık görülen sorunlara kapsamlı bir bakış ve bunlar için çözüm önerileri
output:
  html_document:
    toc: yes
---
...
Bir muhabir^[Bu metin haberciler için hazırlanmış olmakla birlikte aslında veri ile uğraşan herkes için faydalı yönlendirmeler içermektedir.] olarak tüm dünyanız veri ile dolu. Ve bu veriler de problemlerle dolu. Bu kılavuz veriler üzerinde çalışırken karşılaşacağınız birçok sorunla ilgili detaylı açıklamalar ve çözüm önerileri sunmayı hedeflemektedir. 

Bu sorunların çoğu çözülebilir. Bir kısmı ise çözülemez ve bu da ilgili veriyi kullanmamanız gerektiği anlamına gelir. Diğer bir kısımda ise sorunlar çözülemese de veriyi oldukça dikkatli bir şekilde kullanmak mümkündür. Bu belirsizlikleri aşmak için bu kılavuz, sorunları en iyi şekilde çözecek donanıma sahip olan sizler için (siz, kaynağınız, konunun uzmanı) hazırlanmıştır. Her sorunun tanımında eğer ilgili kişi size yardım edemezse neler yapabileceğinize dair öneriler yer almaktadır.

Elde ettiğiniz her veriyi tüm bu sorunlar için gözden geçirmeniz mümkün değildir. Eğer öyle yaparsanız yayın yapamazsınız. Ancak bu sorunlara aşinalık kazanarak, karşılaşabileceğiniz sorunları öngörebilirsiniz. Ve size hata yaptırmadan önce bu sorunları tahmin etme olasılığınızı arttırabilirsiniz.

Bu kılavuz ile ilgili sorularınız varsa lütfen [Chris](mailto:chrisgroskopf@gmail.com)’e Türkçe çevirisi^[Çevirmenin ekleri burada olduğu gibi dipnot olarak verilmiştir] için de [Serdar](drserdarbalci@gmail.com)’a ulaşın. Bol şans!

Bu çalışma [Creative Commons Attribution-NonCommercial 4.0 International License](http://creativecommons.org/licenses/by-nc/4.0/) ile lisanslanmıştır. Önerileriniz için lütfen `pull request` gönderin!

# Çeviriler

* [İngilizce Orijinal](https://github.com/Quartz/bad-data-guide)
* [Çince](http://djchina.org/2016/07/12/bad_data_guide/) (tamamlandı)
* [Çince](http://cn.gijn.org/2016/01/10/quartz%E5%9D%8F%E6%95%B0%E6%8D%AE%E6%8C%87%E5%8D%97%E7%B2%BE%E9%80%89%EF%BC%9A%E5%A4%84%E7%90%86%E6%95%B0%E6%8D%AE%E7%9A%84%E6%AD%A3%E7%A1%AE%E6%96%B9%E5%BC%8F%E4%B8%80%E8%A7%88/) (kısmen)
* [Japonca](https://github.com/piyo-ko/bad-data-guide/blob/master/README_ja.md)
* [Portekizce](http://escoladedados.org/2016/09/08/guia-quartz-para-limpeza-de-dados/)
* [İspanyolca](http://es.schoolofdata.org/guia-quartz/)

Bu kılavuzu kendi dilinize çevirmek ister misiniz? Lütfen çevirin! [Chris](mailto:chrisgroskopf@gmail.com)’e e-posta gönderirseniz çevirinizin bağlantısı buraya eklenecektir.


<!-- ## Kaynağınızın çözmesi gereken sorunlar -->

<!-- * [Kayıp veri](#kayip-veri) -->
...
Herhangi bir veri setindeki boş ya da "null" değerlerin ne anlama geldiğine mutlaka emin olun. Eğer veri yıla ait ise, o yıla ait hiç veri toplanmamış mı? Eğer bir anket ise, katılımcılardan biri soruya cevap vermeyi mi reddetti?


Ne zaman kayıp değerleri olan bir veri üzerine çalışırsanız kendinize şunu sorun: "Bu verinin kayıp olmasının anlamını biliyor muyum?" Eğer cevabınız hayır ise, bunu kaynağınıza sormalısınız.

### Kayıp verilerin yerine sıfır konulması

Kayıp veriden daha kötüsü ise kayıp veriler yerine herhangi bir verinin kullanılmasıdır. Bunun nedeni, etkilerinin ne olacağını tahmin edemeyen bir insan olabileceği gibi, "null" verileri nasıl işleyeceğini bilemeyen bir otomatik süreç de olabilir. Her halde, eğer bir seri halinde sıfırlar görürseniz bunun gerçekten `0` sayısı mı yoksa "hiç bir şey" mi demek olduğunu kendinize sormalısınız (Genelde `-1` "hiç bir şey" anlamında kullanılır). Eğer emin değilseniz, kaynağınıza sorun. Benzer bir özeni `0`'ın farklı şekilde yansıdığı numerik olmayan diğer değerlerde de göstermelisiniz. Örneğin, tarih verisi için `0` değeri hatalı olarak `1970-01-01T00:00:00Z` ya da `1969-12-31T24:59:59Z` ( [Unix epoch for timestamps](https://en.wikipedia.org/wiki/Unix_time#Encoding_time_as_a_number) ) şeklinde görünebilir. Lokalizasyon verisi için `0` hatalı olarak `0°00'00.0"N+0°00'00.0"E` ya da daha basitçe `0°N 0°E` olarak görünebilir, ki burası Gana'nin güneyinde, Atlantik Okyanusunda yer alan ve genellikle [Null Island](https://en.wikipedia.org/wiki/Null_Island) olarak adlandırılan yerdir.
...
=======
>>>>>>> b99ac0cd6f072f098d64590545f8ce6ac4567bb0
### Olması gerektiğini bildiğiniz veriler kayıp
